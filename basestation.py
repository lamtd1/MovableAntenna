...
class BS:
    ...
    def update_system(self, antenna_array, beamforming_matrices):
        ...
        #-------------------
        eva_channel_vector_transpose = self.eva.get_channel_vector().conj().T
        ...
        for i in range(self.cfg.Number_of_com_components):
            # Extent Eavedropper related variable for calculation support
            eva_user_channel_beam_pair = (np.linalg.norm(eva_channel_vector_transpose @ beamforming_matrices[i][:]))**2  
            sum_eva_user = 0

            # Extent User-Basestation variable for calculation support 
